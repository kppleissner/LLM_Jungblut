...
# 1.Combining (concatenate)  Title, author, year and abstract of all
#  papers in one large string
# Read file obtained by script:
# extract_abstracts_from_PubMed_by_easyPubMed_prompted _to_ChatGPT
#
df <- read.csv(
  "I:/My_R_Scripts_2025/LLM_Jungblut/pubmed_results_for_Jungblut.csv",
  stringsAsFactors = FALSE
)

combined_text <- paste(df$Title,df$Author, df$Year, df$Abstract, collapse = " ")


#From here training is launched
# Optional: put it back into a one-row data frame
...
#
#read large string (training set) combined_text
txt <- combined_text
...
train_model(model, training_text, tokenizer, epochs = 800, seq_len = 32, batch_size = 4)
...
#
# output of model
torch_save(model, "I:/My_R_Scripts_2025/LLM_Jungblut/model_total.pt")
model_serialized <-  torch_serialize(model)

reloaded_model <- torch_load("I:/My_R_Scripts_2025/LLM_Jungblut/model_total.pt"
)
####### andere save-methode

torch_save(model$state_dict(), "model.pt")
torch_save( model$state_dict(), "I:/My_R_Scripts_2025/LLM_Jungblut/model-state_dict.pt")


# Later, load it back
state_dict <- torch_load("model.pt")
model_gelesen <- model$load_state_dict(state_dict)

reloaded_model <- torch_load("I:/My_R_Scripts_2025/LLM_Jungblut/model_total.pt")

stop("Stopping script intentionally. Model is saved !")

##################################################################
##################################################################
## ab hier KPP
# prompts <- c("Helicobacter", "host",  "bacteria", "infection disease", "electron microscopy")
#prompts <- c("what are the most important scientific impacts of Meyer TF on helicobacter reaearch")
prompts <- c("Which is the most cited  publication ?")
for (i in 1:length(prompts)) {
  generated <- generate_text(model, tokenizer, prompt = prompts[i], max_new_tokens = 20, temperature = 0.8)
  cat(sprintf("Prompt> '%s':\n\n%s\n", prompts[i], generated))
}

prompts <- c(" Count the word Helicobacter ")
for (i in 1:length(prompts)) {
  generated <- generate_text(model, tokenizer, prompt = prompts[i], max_new_tokens = 20, temperature = 0.8)
  cat(sprintf("Prompt> '%s':\n\n%s\n", prompts[i], generated))
}
...
prompts <- c("how is the most cited author ?  ")
for (i in 1:length(prompts)) {
  generated <- generate_text(model, tokenizer, prompt = prompts[i], max_new_tokens = 20, temperature = 0.8)
  cat(sprintf("Prompt> '%s':\n\n%s\n", prompts[i], generated))
}
